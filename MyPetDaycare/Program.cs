...
Console.Write("Hello, Welcome to My Pet Daycare! What will you name your brand new pet?\n> ");
pet.Name = NullValidation(Console.ReadLine(), "Please choose your pet's name!");

//formats name: upper the first letter, lower the rest
pet.Name = char.ToUpper(pet.Name[0]) + pet.Name.Substring(1).ToLower();
...
void MainMenu()
{
    //manages the main loop with display
    Console.Clear();

    pet.PetDisplay();

    string[] stats = pet.GenerateStats();
    foreach (string stat in stats)
    {
        Console.WriteLine(stat);
    }

    Console.WriteLine($"\n{pet.GetStatusMessage()}\n");

    bool makingDecisions = true;

    Console.Write($"What would you like to do with {pet.Name} now?\n" +
                   "[1]Play     [2]Feed     [3]Rest     [4]Save Pet      [5]Load Pet     [6] Quit\n\n> ");

    while (makingDecisions)
    {
        string playerChoice = NullValidation(Console.ReadLine(), $"Choose what you want to do with {pet.Name}.");

        switch (playerChoice)
        {
            case "1":
                pet.Play();
                Console.WriteLine($"You played with {pet.Name}! {pet.PlayReaction()}\n");
                makingDecisions = false;
                break;
            case "2":
                pet.Feed();
                Console.WriteLine($"You fed {pet.Name}! {pet.FeedReaction()}\n");
                makingDecisions = false;
                break;
            case "3":
                pet.Sleep();
                Console.WriteLine($"{pet.Name} went to sleep! {pet.SleepReaction()}\n");
                makingDecisions = false;
                break;
            case "4":
                pet.SaveGame();
                ...
                break;
            case "5":
                Console.Write("Enter the filename where your pet is located!\n" +
                              "The format is \"[PET NAME]'s_Save.txt\"\n> ");
                string saveFile = NullValidation(Console.ReadLine(), "Enter the filename of your pet!\n" +
                                                                     "The format is \"[PET NAME]'s_Save.txt\"\n> ");
                pet.LoadGame(saveFile);
                Console.WriteLine("\nPress any key to continue...");
                Console.ReadKey();
                makingDecisions = false;
                break;
            case "6":
                programOn = false;
                makingDecisions = false;
                Console.WriteLine($"Thank you for visiting MyPetDaycare and taking care of {pet.Name}! See you again!");
                return;
            default:
                Console.Write($"Huh? What do you want to do with {pet.Name}?\n" +
                               "[1]Play     [2]Feed     [3]Rest     [4]Save Pet     [5]Load Pet     [6]Quit\n> ");
                continue;
        }
    }
    programOn = pet.CheckGameOver();
}
string NullValidation(string input, string repromptUser)
{
